...
// Copyright (c) 2021-2023 The Flincoin Core developers
...
{
    // Flincoin: Never mine witness tx
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
            "2. address      (string, required) The address to send the newly generated flincoin to.\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Flincoin is downloading blocks...");
...
static UniValue AuxMiningCreateBlock(const CScript& scriptPubKey)
{
     ...
     * a single flincoind instance, for example when a pool runs multiple sub-
